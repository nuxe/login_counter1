# -*- encoding: utf-8 -*-

Gem::Specification.new do |s|
  s.name = "rspec-rails"
  s.version = "3.0.0.beta1"

  s.required_rubygems_version = Gem::Requirement.new("> 1.3.1") if s.respond_to? :required_rubygems_version=
  s.authors = ["David Chelimsky", "Andy Lindeman"]
  s.cert_chain = ["-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n"]
  s.date = "2013-11-08"
  s.description = "RSpec for Rails"
  s.email = "rspec@googlegroups.com"
  s.homepage = "http://github.com/rspec/rspec-rails"
  s.licenses = ["MIT"]
  s.rdoc_options = ["--charset=UTF-8"]
  s.require_paths = ["lib"]
  s.rubyforge_project = "rspec"
  s.rubygems_version = "2.0.3"
  s.summary = "rspec-rails-3.0.0.beta1"

  if s.respond_to? :specification_version then
    s.specification_version = 4

    if Gem::Version.new(Gem::VERSION) >= Gem::Version.new('1.2.0') then
      s.add_runtime_dependency(%q<activesupport>, [">= 3.0"])
      s.add_runtime_dependency(%q<activemodel>, [">= 3.0"])
      s.add_runtime_dependency(%q<actionpack>, [">= 3.0"])
      s.add_runtime_dependency(%q<railties>, [">= 3.0"])
      s.add_runtime_dependency(%q<rspec-core>, ["= 3.0.0.beta1"])
      s.add_runtime_dependency(%q<rspec-expectations>, ["= 3.0.0.beta1"])
      s.add_runtime_dependency(%q<rspec-mocks>, ["= 3.0.0.beta1"])
      s.add_runtime_dependency(%q<rspec-collection_matchers>, [">= 0"])
      s.add_development_dependency(%q<rake>, ["~> 10.0.0"])
      s.add_development_dependency(%q<cucumber>, ["~> 1.3.5"])
      s.add_development_dependency(%q<aruba>, ["~> 0.4.11"])
      s.add_development_dependency(%q<ZenTest>, ["= 4.9.4"])
      s.add_development_dependency(%q<ammeter>, ["= 0.2.5"])
    else
      s.add_dependency(%q<activesupport>, [">= 3.0"])
      s.add_dependency(%q<activemodel>, [">= 3.0"])
      s.add_dependency(%q<actionpack>, [">= 3.0"])
      s.add_dependency(%q<railties>, [">= 3.0"])
      s.add_dependency(%q<rspec-core>, ["= 3.0.0.beta1"])
      s.add_dependency(%q<rspec-expectations>, ["= 3.0.0.beta1"])
      s.add_dependency(%q<rspec-mocks>, ["= 3.0.0.beta1"])
      s.add_dependency(%q<rspec-collection_matchers>, [">= 0"])
      s.add_dependency(%q<rake>, ["~> 10.0.0"])
      s.add_dependency(%q<cucumber>, ["~> 1.3.5"])
      s.add_dependency(%q<aruba>, ["~> 0.4.11"])
      s.add_dependency(%q<ZenTest>, ["= 4.9.4"])
      s.add_dependency(%q<ammeter>, ["= 0.2.5"])
    end
  else
    s.add_dependency(%q<activesupport>, [">= 3.0"])
    s.add_dependency(%q<activemodel>, [">= 3.0"])
    s.add_dependency(%q<actionpack>, [">= 3.0"])
    s.add_dependency(%q<railties>, [">= 3.0"])
    s.add_dependency(%q<rspec-core>, ["= 3.0.0.beta1"])
    s.add_dependency(%q<rspec-expectations>, ["= 3.0.0.beta1"])
    s.add_dependency(%q<rspec-mocks>, ["= 3.0.0.beta1"])
    s.add_dependency(%q<rspec-collection_matchers>, [">= 0"])
    s.add_dependency(%q<rake>, ["~> 10.0.0"])
    s.add_dependency(%q<cucumber>, ["~> 1.3.5"])
    s.add_dependency(%q<aruba>, ["~> 0.4.11"])
    s.add_dependency(%q<ZenTest>, ["= 4.9.4"])
    s.add_dependency(%q<ammeter>, ["= 0.2.5"])
  end
end
